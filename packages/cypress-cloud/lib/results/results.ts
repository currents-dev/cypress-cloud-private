...
import { TestAttempt, TestHook } from "../runner/spec.type";
...
export const getInstanceTestsPayload = (
	runResult: CypressCommandLine.RunResult,
	config: CypressCommandLine.PublicConfig
): SetInstanceTestsPayload => {
	return {
		config: {
			...config,
		...
		tests:
			runResult.tests?.map((test, i) => ({
				...
				config: null,
				body:
					(test as CypressCommandLine.TestResult & { body: string }).body ??
					"redacted",
				clientId: `r${i}`,
				hookIds: [],
			})) ?? [],
		hooks:
			(runResult as CypressCommandLine.RunResult & { hooks: TestHook[] })
				.hooks ?? [],
	};
};
